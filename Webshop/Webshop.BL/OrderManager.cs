...
namespace Webshop.BL
{
	public class OrderManager
	{
		...
		public async Task<(bool, OrderDetails?)> GetOrder(int orderId, string userId)
		{
			return await orderRepository.GetOrderDetails(orderId, userId);
		}

		public async Task<IReadOnlyCollection<Order>> GetOrders(string userId)
		{
		...
		public async Task<bool> TryCancelOrder(int orderId, string userId)
		{
			using (var transaction = new TransactionScope(
						TransactionScopeOption.Required,
						new TransactionOptions() { IsolationLevel = IsolationLevel.RepeatableRead },
						TransactionScopeAsyncFlowOption.Enabled))
			{
				var productStocks = await orderRepository.CancelOrder(orderId, userId);
				if (productStocks.Count > 0)
				{
					if (await productManager.AddStock(productStocks))
					{
						transaction.Complete();
						return true;
					}
